...
export default {
  ...
  computed: {
    titleStyle() {
      return {
        transform: this.timeElapsed > 4_500 ? 'translateY(-15px)' : 'translateY(175px)'
      }
    },
    gifStyle() {
      return {
        transform: this.timeElapsed > 4_500 ? 'translateY(0px)' : 'translateY(125px)',
        opacity: this.timeElapsed > 4_500 ? 1 : 0
      }
    }
  }
}
